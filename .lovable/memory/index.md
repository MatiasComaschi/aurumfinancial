...
Auth: Google OAuth (Lovable Cloud managed) + email/password via Supabase auth. Password reset via /reset-password route.
...
DB tables: profiles, plaid_items, goals, bill_overrides, user_memory, user_preferences (synced prefs replacing localStorage).
Transactions: Fetch 60 days from Plaid, show 30 in UI, feed 60 to AI/bill detection. Search/filter by merchant, category, date range, amount.
Balances: Pull from Plaid accounts response, display per-account cards on Overview. Visibility synced to user_preferences table.
...
Onboarding: 3-step flow (welcome, connect bank, how it works) for new users without linked banks. Progress tracked in user_preferences.
